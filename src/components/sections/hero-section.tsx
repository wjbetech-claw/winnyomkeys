...
export function HeroSection({
  ...
  const koreanTitleLines =
    locale === "ko"
      ? (() => {
          const [firstLine, ...rest] = title.split(" ");
          return rest.length > 0 ? [firstLine, rest.join(" ")] : [title];
        })()
      : null;
  ...
  return (
    ...
    >
      <div className="grid gap-4 animate-[rise-in_720ms_ease_both]">
        <h1
          className={`text-[clamp(2.9rem,6vw,6.3rem)] font-bold text-balance ${
            locale === "ko"
              ? "ko-tight-balance max-w-[12.8ch] leading-[1.03] tracking-[-0.035em] sm:max-w-[11.6ch]"
              : "max-w-[10ch] leading-[0.9] tracking-normal"
          }`}
        >
          {koreanTitleLines ? (
            <>
              <span className="block">{koreanTitleLines[0]}</span>
              {koreanTitleLines[1] ? (
        ...
        <p
          className={`max-w-[58ch] text-[clamp(1.05rem,2vw,1.22rem)] text-(--soft-text) ${
            locale === "ko"
              ? "ko-copy leading-[1.82] tracking-[-0.01em]"
              : "leading-[1.75]"
          }`}
        >
          {subtitle}
      ...
      >
        ...
        <figure className="relative z-10 aspect-[0.9] w-full max-w-140 animate-[float_10s_ease-in-out_infinite] overflow-hidden rounded-[34px] border border-[rgba(255,255,255,0.1)] shadow-[0_28px_70px_rgba(0,0,0,0.32),0_10px_28px_rgba(0,0,0,0.18)]">
          ...
          <figcaption className="absolute bottom-6 left-6 right-6 grid max-w-[30ch] gap-3 text-[#f6f3ee]">
            <p
              className={`m-0 text-[1rem] text-[rgba(246,243,238,0.9)] ${
                locale === "ko" ? "ko-copy leading-[1.72]" : "leading-[1.6]"
              }`}
            >
              {imageCaption}
            </p>
