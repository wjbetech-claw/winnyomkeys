...
function WinnyomKeysMark() {
  return (
    <span
      className="relative h-12.5 w-12.5 shrink-0 rounded-xl border border-[rgba(63,56,45,0.55)] bg-[linear-gradient(180deg,#d8d0bf_0%,#b8ae98_100%)] p-0.75 shadow-[inset_0_1px_0_rgba(255,255,255,0.45),0_8px_18px_rgba(0,0,0,0.22)]"
      aria-hidden="true"
    >
    ...
    <main className="relative mx-auto w-full max-w-295 px-4 pb-10 pt-26 sm:px-6 lg:px-10 before:pointer-events-none before:fixed before:inset-0 before:content-[''] before:opacity-35 before:bg-[linear-gradient(to_right,rgba(255,255,255,0.025)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.025)_1px,transparent_1px)] before:bg-size-[72px_72px] before:mask-[radial-gradient(circle_at_center,black_28%,transparent_82%)]">
      ...
      <section className="mt-8 grid gap-4" aria-labelledby="faq-page-title">
        <div className="grid gap-2">
          ...
          <h1
            id="faq-page-title"
            className="max-w-[18ch] text-[clamp(1.9rem,4vw,3.2rem)] font-bold leading-[1.04] tracking-[-0.05em] text-balance"
          >
        ...
        <div className="grid gap-4">
          {faqs.map((faq) => (
            <details
              ...
              key={faq.question}
            >
              <summary className="cursor-pointer list-none text-[1.12rem] font-semibold leading-[1.35] [&::-webkit-details-marker]:hidden">
                {faq.question}
              </summary>
              <p className="mt-3.5 leading-[1.72] text-(--soft-text)">
                {faq.answer}
              </p>
            </details>
          ))}
